...
SocketIO.prototype.handlerBasic = function (resolve, reject) {
  ...
  // Close event
  self.SocketClient.on('disconnect', () => {
    // Error handlers
    self.OnClose();
    // Reconnected
    setTimeout(() => {
...
// React override
SocketIO.prototype.componentWillMount = function () {
  // This
  const self = this;
  // Call parent
  Protocols.prototype.componentWillMount.bind(self)();
  // Send subscribes
  self.Subscribe(this.subscribes || []);
};
// Export
