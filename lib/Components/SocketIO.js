...
SocketIO.prototype.handlerBasic = function (resolve, reject) {
  ...
  // Handler events
  // Error handler
  self.SocketServer.on('error', (myError) => {
    // Error handlers
    reject(myError);
  ...
  const response = (resolve, reject) => {
    ...
    if (!self.SocketServer) {
      ...
      if (ConnectionsPool[serverRoute]) {
        // Pass by pointer
        self.SocketServer = ConnectionsPool[serverRoute];
        // Register pool
        registerAtPool(self);
        // Return server
        ...
        self.SocketServer = socketIo(serverRoute);
        // Open connection
        self.SocketServer.on('connect', (socket) => {
          // Socket client
